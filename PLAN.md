...
Data: `build.clj` platform-archive targets, Vis' root `deps.edn` pin,
...
extracted once. Linux archives also carry a pinned, static-musl bubblewrap at
`bin/bwrap`; the sandbox resolves it beside the cdylib and never depends on a
system package or `PATH`. The artifact carries no bytecode: `__pycache__` is
