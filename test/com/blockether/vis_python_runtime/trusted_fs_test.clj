...
(harness/defbuilt-test
  the-trusted-session-reaches-what-the-policy-refuses-test
  (let [outside
        (temp-dir "vis-trusted-outside")

        inside
        (temp-dir "vis-trusted-inside")
    ...
    (testing "the trusted session reads it, writes it, and lists its directory"
      (let [answer (block extension
                          (str "import _vis_fs\n"
                               ...
                               ").decode())\n"
                               "_vis_fs.write("
                               (pr-str (str outside "/made.txt"))
                               ", 'made')\n"
                               "print(_vis_fs.read("
                               (pr-str (str outside "/made.txt"))
                               ").decode())\n"
                               "print(sorted(_vis_fs.list("
                               (pr-str outside)
                               ")))"))]
        (is (nil? (:error answer)))
