...
(defn- options
  [^Path root & [extra]]
  (merge {:environment {"PATH" "/usr/bin:/bin" "HOME" (str root) "TMPDIR" (str root) "LANG" "C"}
          :directory (str root)
...
(defn- loopback-server ^ServerSocket [] (ServerSocket. 0 8 (InetAddress/getLoopbackAddress)))
...
(deftest seatbelt-compiler-test
  (let [root
        ...
        compile
        #(Seatbelt/compile (runtime/jail-policy (merge {:read-write [(str root)]} %)))]

    (try (testing "every profile imports the platform and starts from deny"
           (let [p (compile {})]
         ...
         (testing "the deny lists come after the allows so they win"
           (let [sub
                 ...
                 p
                 (compile {:deny-write [sub] :deny-read [sub] :deny-exec [sub]})]

             (is (< (str/index-of p "(allow file-read* file-write*")
                    (str/index-of p "(deny file-write*")))
         ...
         (testing "ancestors of a granted root are metadata literals, never subpaths"
           ...
           ;; look at ITS parent.
           (let [nested
                 (Files/createDirectories (.resolve root "nested") (make-array FileAttribute 0))

                 p
                 (compile {:read-write [(str nested)]})]
         ...
         (testing "network: loopback listeners always, one proxy or nothing outbound"
           (let [off
                 ...
                 proxied
                 (compile {:network {:proxy 5555} :inbound [4200]})]
         ...
         (testing "one exact Unix control socket is reachable after the network deny"
           (let [socket
                 ...
                 p
                 (compile {:unix-connect [socket]})]

             (is (str/includes? p (str "(remote unix-socket(path \"" socket "\"))")))
             (is (< (str/index-of p "(deny network*)") (str/index-of p "(remote unix-socket")))))
...
(deftest bubblewrap-compiler-test
  (let [root
        ...
        compile
        #(vec (Bubblewrap/compile (runtime/jail-policy (merge {:read-write [(str root)]} %))))]

    (try (testing "the argument list is bubblewrap's policy flags ending in --"
           (let [args (compile {})]
         ...
         (testing "deny-read masks a directory with tmpfs and a file with /dev/null"
           (let [dir
                 (str (Files/createDirectory (.resolve root "hidden") (make-array FileAttribute 0)))

                 file
                 (str (Files/createFile (.resolve root "secret") (make-array FileAttribute 0)))

                 args
                 (compile {:deny-read [dir file]})]
...
(deftest pipes-environment-and-seatbelt-or-bubblewrap-test
  (let [root
        ...
        inside
        (.resolve root "inside.txt")]

    (try (testing "ordinary process semantics survive the native boundary"
           (let [result (run-process ["/bin/sh" "-c"
                                      (str "printf '%s' \"$VALUE\" > "
                                           inside
         ...
         (testing "a confined child is marked so a descendant never applies a second profile"
           (is (= "1\n"
                  (:out (run-process ["/bin/sh" "-c" "echo $VIS_SEATBELT_ACTIVE"]
                                     (options root))))))
         ...
         (testing "a deny-read path under a granted root stays unreadable"
           (let [secret
                 ...
                 _
                 (spit (str secret) "hidden")

                 result
                 (run-process ["/bin/sh" "-c" (str "cat " secret)]
                              (options root
                                       {:policy {:read-write [(str root)]
         ...
         (testing "the process is a detached group leader and can be terminated as a tree"
           (let [p (runtime/spawn-process! ["/bin/sh" "-c" "sleep 30 & wait"] (options root))]
             (.destroyForcibly p)
...
(deftest filtered-egress-crosses-only-the-native-loopback-bridge-test
  (let [root
        ...
        allowed-port
        (.getLocalPort allowed)

        denied-port
        (.getLocalPort denied)

        responder
        (future (with-open [socket
                            (.accept allowed)
                  ...
                  (when (= "ping" (.readLine reader)) (.write writer "pong\n") (.flush writer))
                  :responded))]

    (try (let [script
               (str "exec 3<>/dev/tcp/127.0.0.1/"
                    allowed-port
                    "\n"
                    "printf 'ping\\n' >&3\n"
                    "IFS= read -r reply <&3\n"
                    "if exec 4<>/dev/tcp/127.0.0.1/"
                    denied-port
                    " 2>/dev/null; "
               ...
               opts
               (options root {:policy {:read-write [(str root)] :network {:proxy allowed-port}}})

               result
               (run-process ["/bin/bash" "-c" script] opts)]
         ...
         (finally (.close allowed) (.close denied) (delete-tree root)))))
...
(deftest unconfined-process-still-uses-native-lifecycle-test
  (let [root (temp-dir)]
    (try (let [result (run-process ["/bin/sh" "-c"
                                    "printf %s \"$VALUE\"; printf %s \"$VIS_SEATBELT_ACTIVE\""]
