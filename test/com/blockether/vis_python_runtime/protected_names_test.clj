...
(harness/defbuilt-test shadowing-a-tool-is-block-local-test
  (let [session (harness/tool-session {"patch" patched})
        shadow  (block session "patch = 'not callable'\nprint(patch)")]
    (testing "the assignment itself is allowed and the block reads its own string"
      (is (nil? (:error shadow)))
...
(harness/defbuilt-test read-before-the-shadow-sees-the-tool-test
  ...
  (let [session (harness/tool-session {"patch" patched})]
    (testing "a read that precedes the shadowing assignment still reaches the tool"
      (is (= "patched:x shadow"
             (ran session (str "before = await patch('x')\n"
                               "patch = 'shadow'\n"
                               "print(before, patch)")))))))

(harness/defbuilt-test ordinary-variables-alongside-a-tool-test
  (let [session (harness/tool-session {"patch" patched})]
    (testing "a variable that is not a bound name is an ordinary global"
      (is (= "patched:app.css"
             (ran session (str "css = 'app.css'\n"
...
(harness/defbuilt-test only-bound-names-are-protected-test
  ...
  (let [session (harness/tool-session {"patch" patched})]
    ...
    (testing "so binding them is an ordinary assignment that survives the block"
      (is (= "patched:promise_pool.test.ts"
             (ran session (str "test = 'promise_pool.test.ts'\n"
                               "format = 'csv'\n"
...
(harness/defbuilt-test print-survives-a-block-that-shadows-it-test
  ...
  (let [session (harness/tool-session {"patch" patched})
        shadow  (block session "print = 'not callable'")]
...
(harness/defbuilt-test loop-target-shadowing-a-tool-test
  ...
  (let [session (harness/tool-session {"patch" patched})
        looped  (block session "for patch in ['a', 'b']:\n    pass")]
    (testing "the loop is allowed to use the name"
      (is (nil? (:error looped))))
...
(harness/defbuilt-test deleting-a-tool-is-block-local-test
  ...
  (let [session (harness/tool-session {"patch" patched})
        deleted (block session "del patch")]
...
(harness/defbuilt-test top-level-def-over-a-tool-is-refused-test
  ...
  (let [session (harness/tool-session {"patch" patched})
        refused (block session "def patch(a):\n    return a\n")
        klass   (block session "class defs:\n    pass\n")]
    ...
    (testing "a def NESTED in another function is an ordinary local, never refused"
...
(harness/defbuilt-test restore-never-recreates-a-tool-name-test
  ...
  (let [session (harness/tool-session {"patch" patched})
        snapshot (str "def patch(*a, **k):\n    return \"HIJACKED\"\n\n"
                      "defs = \"clobbered\"\n\n"
                      "def kept(n):\n    return n * 3\n")
        restored (harness/ev session
                             (str "__vis_restore_defs__(" (json/write-str snapshot) ")"))]
    ...
    (testing "the restore says which names it dropped, so the file can heal"
      (is (= ["defs" "patch"]
             (harness/ev session "__vis_restore_dropped__"))))))
