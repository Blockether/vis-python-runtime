...
(defn- across-processes
  ...
  [setup probe]
  (let [written  (harness/block-session)
        _        (block written setup)
        text     (snapshot written)
        fresh    (harness/block-session)
        restored (restore! fresh text)]
    {:snapshot text :restored restored :stdout (ran fresh probe)}))

(harness/defbuilt-test session-defs-round-trip-test
  (let [{:keys [snapshot restored stdout]}
        (across-processes
         (str "ROOT = \"/tmp/vis-defs-probe\"\n"
              "import json as J\n"
              "def shout(s):\n    return s.upper()\n")
         (str "print(shout(\"ok\"))\n"
...
(harness/defbuilt-test session-defs-listed-as-restored-test
  (let [written (harness/block-session)
        _       (block written "def widen(a, b=2):\n    return a * b\n")
        fresh   (harness/block-session)
        _       (restore! fresh (snapshot written))]
...
(harness/defbuilt-test session-defs-partial-restore-test
  (testing "every definition that still loads survives one statement that does not"
    ...
    ;; by statement.
    (let [fresh    (harness/block-session)
          restored (restore! fresh (str "import totally_missing_module as tm\n"
  ...
  (testing "a snapshot that will not parse answers zero and leaves the sandbox usable"
    (let [fresh    (harness/block-session)
          restored (restore! fresh "def broken(:\n    ???\n")]
      (is (= 0 restored))
      (is (= "2" (ran fresh "print(1 + 1)"))))))

(harness/defbuilt-test session-defs-nested-and-aliased-test
  ...
  (let [{:keys [snapshot restored stdout]}
    ...
    (testing "the whole toolbox comes back when a helper is nested or bound under another name"
      (is (= 4 restored))
      (is (= "8 2 2" stdout)))
    (testing "a closure is rebound to the name the session calls it by, and the private name dropped"
...
(harness/defbuilt-test session-defs-carries-classes-and-config-test
...
(harness/defbuilt-test session-defs-restores-with-the-block-rewrite-test
...
(harness/defbuilt-test defs-verb-test
  ...
  ;; `inspect`.
  (let [session (harness/block-session)
        empty   (ran session "print(defs())")
        _       (block session "from json import dumps\ndef widen(a, b=2):\n    return a * b\n")
        listed  (ran session "print(defs())")
        source  (ran session "print(defs(\"widen\"))")
        missing (ran session (str "try:\n"
                                  "    defs(\"nope\")\n"
...
(harness/defbuilt-test defs-docstring-surface-test
  ...
  (let [session (harness/block-session)
        _ (block session (str "def kebab_to_snake(text):\n"
                              ...
                              "def quiet(x):\n    return x\n"))
        listed (ran session "print(defs())")
        docs   (harness/printed (block session "print(json.dumps(__vis_def_docs__()))"))
        calls  (harness/printed (block session "print(json.dumps(__vis_def_calls__()))"))]
...
(harness/defbuilt-test tool-shadow-refusal-test
  ...
  (let [session (harness/tool-session {"shadow_probe" (fn [_] "REAL-TOOL")})]
    ...
    (testing "a def nested in another function is an ordinary local, not a shadow"
      (is (= "7" (ran session (str "def outer():\n"
                                   "    def defs(x):\n        return x\n"
                                   "    return defs(7)\n"
                                   "print(outer())\n")))))
...
(harness/defbuilt-test restore-never-overwrites-a-bound-tool-test
  ...
  (let [session  (harness/tool-session {"probe_tool" (fn [_] "REAL-TOOL")})
        restored (restore! session (str "def probe_tool(*a, **k):\n    return \"HIJACKED\"\n\n"
                                        "defs = \"clobbered\"\n"
                                        "def kept(n):\n    return n * 3\n"))
        out      (ran session (str "print(kept(2))\n"
                                   "print(\"HIJACKED\" in str(defs()))\n"
                                   "print(callable(defs))\n"
                                   "print(await probe_tool(\"x\"))\n"))
        dropped  (harness/printed (block session "print(json.dumps(__vis_restore_dropped__))"))]
