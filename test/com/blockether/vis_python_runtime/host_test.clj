...
(defn- echo "The tool every case here reaches for: `<x>`." [args] (str "<" (first args) ">"))
...
(harness/defbuilt-test host-tool-deferred-test
                       ...
                       (let [calls
                             ...
                             session
                             (harness/tool-session {"tick" (fn [_]
                                                             (swap! calls inc))
                                                    "echo" echo})]
                         ...
                         (testing "gather settles two host calls in one block"
                           (is (= "['<a>', '<b>']"
                                  (ran session "print(await gather(echo('a'), echo('b')))"))))))

(harness/defbuilt-test
  host-failure-catchable-test
  (let [session (harness/tool-session {"boom" (fn [_]
                                                (throw (ex-info "host said no" {})))
                                       "echo" echo})]
...
(harness/defbuilt-test
  ...
  (let [session (harness/tool-session {"echo" echo})]
    ...
    (testing "a name the host does not know is the host's answer, not a crash"
      (is (str/includes? (harness/ev session
...
(harness/defbuilt-test unbound-host-test
                       ...
                       (let [session (harness/tool-session {"echo" echo})]
                         (try (runtime/bind-host! nil)
                              (is (str/includes? (str (:error (block session
                                                                     "print(await echo('x'))")))
                                                 "no host is bound"))
                              (finally (harness/bind-tools! {"echo" echo})))
...
(harness/defbuilt-test host-tool-names-its-session-test
                       ...
                       (let [seen (atom [])]
                         (try (runtime/bind-host! (fn [session nm _payload]
...
(harness/defbuilt-test
  gathered-host-tools-keep-session-and-options-test
  ...
  (let [seen (atom [])]
    (try (runtime/bind-host! (fn [session nm payload]
...
(harness/defbuilt-test
  the-caller-is-the-interpreters-answer-test
